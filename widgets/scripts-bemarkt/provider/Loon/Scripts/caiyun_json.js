...
$request.url.includes("/v2/user")
  ? (((responseBody = JSON.parse($response.body)).result.is_vip = 1),
  ...
  : $request.url.includes("operation/homefeatures")
  ? (responseBody = { data: [] })
  : $request.url.includes("operation/feeds")
  ? ((responseBody = JSON.parse($response.body)).data =
      responseBody.data.filter(
        (e) => -1 != e.category_times_text.indexOf("人查看")
      ))
  : $request.url.includes("operation/banners")
  ? (responseBody = {
      data: [
        {
          avatar:
  ...
  : $request.url.includes("operation/features")
  ? ((responseBody = JSON.parse($response.body)).data =
      responseBody.data.filter((e) => -1 != e.url.indexOf("cy://")))
      ...
      campaigns: [
        {
          ...
          cover:
            "https://cdn-w.caiyunapp.com/p/banner/test/668d442c4fe75aca7251c161.png",
        },
      ],
    })
  : $request.url.includes("notification/message_center")
  ? (responseBody = { messages: [] })
  : $request.url.includes("config/cypage")
