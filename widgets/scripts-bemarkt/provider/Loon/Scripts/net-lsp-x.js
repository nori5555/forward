...
if (
  ...
  $.lodash_get($environment, "executor") === "event-network"
) {
...
!(async () => {
  ...
  if ($.lodash_get(arg, "TYPE") === "EVENT") {
    ...
    if (
      ...
      // 有任何一项不同 都继续
      $.setjson(
        { CN_IP, PROXY_IP, CN_IPv6, PROXY_IPv6 },
        "lastNetworkInfoEvent"
      );
    } else {
      // 否则 直接结束
      $.log("网络信息未发生变化, 不继续");
  ...
  if (continueFlag) {
    ...
    if (ENTRANCE_IP && ENTRANCE_IP !== PROXY_IP) {
      const entranceDelay = parseFloat(
        $.lodash_get(arg, "ENTRANCE_DELAY") || 0
      );
      $.log(
        `入口: ${ENTRANCE_IP} 与落地 IP: ${PROXY_IP} 不一致, 等待 ${entranceDelay} 秒后查询入口`
      );
      if (entranceDelay) {
        await $.wait(1000 * entranceDelay);
      }
      let [
        { CN_INFO: ENTRANCE_INFO1 = "", isCN = false } = {},
        { PROXY_INFO: ENTRANCE_INFO2 = "" } = {},
      ] = await Promise.all([
    ...
    title = `${PROXY_POLICY}`;
    content = `${SSID}${LAN}${CN_POLICY}IP: ${
      maskIP(CN_IP) || "-"
    }${CN_IPv6}${maskAddr(CN_INFO)}\n\n${ENTRANCE}落地 IP: ${
      maskIP(PROXY_IP) || "-"
      ...
      if ($.lodash_get(arg, "TYPE") === "EVENT") {
        await notify(
          `🄳 ${maskIP(CN_IP) || "-"} 🅿 ${maskIP(PROXY_IP) || "-"}`
            ...
            .trim(),
          `${maskAddr(
            CN_INFO.replace(/(位置|运营商).*?:/g, "").replace(/\n/g, " ")
          )}`
            .replace(/\n+/g, "\n")
            .replace(/\ +/g, " ")
            .trim(),
          `${maskAddr(
            PROXY_INFO.replace(/(位置|运营商).*?:/g, "").replace(/\n/g, " ")
          )}${CN_IPv6 ? `\n🄳 ${CN_IPv6.replace(/\n+/g, "")}` : ""}${
            PROXY_IPv6 ? `\n🅿 ${PROXY_IPv6.replace(/\n+/g, "")}` : ""
          }${SSID ? `\n${SSID}` : "\n"}${LAN}`
            .replace(/\n+/g, "\n")
      ...
      const html = `<div style="font-family: -apple-system; font-size: large">${`\n${content}${
        proxy_policy
          ? `\n\n${proxy_policy.replace(
              /^(.*?:\s*)(.*)$/,
              '$1<span style="color: #467fcf">$2</span>'
            )}`
          : ""
      }`
...
async function getProxyRequestInfo({ PROXIES = [] } = {}) {
  const { PROXY_IP, PROXY_INFO, PROXY_PRIVACY } = await getProxyInfo(
    undefined,
    $.lodash_get(arg, "LANDING_IPv4")
  );
  let result;
  if ($.isSurge() || $.isStash()) {
    result = await getRequestInfo(
    ...
    if ($.isSurge()) {
      ...
      const connection =
        connections.slice(0, 10).find((i) => {
          const dest =
            $.lodash_get(i, "metadata.host") ||
            $.lodash_get(i, "metadata.destinationIP");
...
async function getDirectInfo(ip, provider) {
  ...
  if (provider == "cip") {
    ...
    try {
      ...
      CN_INFO = [
        [
          ...
          .filter((i) => i)
          .join(" "),
        ["运营商:", $.lodash_get(body, "data.owner")]
          .filter((i) => i)
          .join(" "),
      ]
        .filter((i) => i)
    ...
    try {
      ...
      CN_INFO = [
        [
          ...
          .filter((i) => i)
          .join(" "),
        ["运营商:", $.lodash_get(body, "data.location.4")]
          .filter((i) => i)
          .join(" "),
      ]
        .filter((i) => i)
    ...
    try {
      ...
      CN_INFO = [
        [
          ...
          .filter((i) => i)
          .join(" "),
        ["运营商:", $.lodash_get(body, "result.operator")]
          .filter((i) => i)
          .join(" "),
      ]
        .filter((i) => i)
    ...
    try {
      ...
      CN_INFO = CN_INFO = [
        [
          "位置:",
          getflag(countryCode),
          $.lodash_get(body, "desc").replace(/中国\s*/, ""),
        ]
          .filter((i) => i)
          .join(" "),
        $.lodash_get(arg, "ORG") == 1
          ? ["组织:", $.lodash_get(body, "org") || "-"]
              .filter((i) => i)
              .join(" ")
          : undefined,
      ]
    ...
    try {
      ...
      const countryCode = $.lodash_get(
        body,
        "data.showapi_res_body.en_name_short"
      );
      isCN = countryCode === "CN";
      ...
      CN_INFO = [
        [
          "位置:",
          getflag(countryCode),
          $.lodash_get(body, "data.showapi_res_body.country").replace(
            /\s*中国\s*/,
            ""
          ),
          ...
          .filter((i) => i)
          .join(" "),
        ["运营商:", $.lodash_get(body, "data.showapi_res_body.isp") || "-"]
          .filter((i) => i)
          .join(" "),
      ]
        .filter((i) => i)
    ...
    try {
      ...
      if (!APPCODE)
        throw new Error("请在 DOMESTIC_IPv4_KEY 填写阿里云 IP 接口的 APPCODE");
      ...
      if (!ali_ip) {
        ...
        if (!ali_ip)
          throw new Error("阿里云接口需要 IP. 未提供 IP, 使用默认 IP 查询失败");
      }
    ...
    try {
      ...
      CN_INFO = [
        ...
        [
          "运营商:",
          $.lodash_get(body, "data.operator") ||
            $.lodash_get(body, "data.isp") ||
            "-",
        ]
          .filter((i) => i)
          .join(" "),
...
async function getDirectInfoIPv6() {
  let CN_IPv6;
  const msg = `使用 ${
    $.lodash_get(arg, "DOMESTIC_IPv6") || "ddnspod"
  } 查询 IPv6 分流信息`;
  ...
  if (provider == "ipinfo") {
    ...
    try {
      ...
      PROXY_INFO = [
        ...
        [
          ...
          .filter((i) => i)
          .join(" "),
        ["运营商:", body.isp || body.org || body.asn]
          .filter((i) => i)
          .join(" "),
      ]
        .filter((i) => i)
    ...
    try {
      ...
      PROXY_INFO = [
        ...
        $.lodash_get(arg, "ORG") == 1
          ? ["组织:", $.lodash_get(body, "asn_organization") || "-"]
              .filter((i) => i)
              .join(" ")
          : undefined,

        $.lodash_get(arg, "ASN") == 1
          ? ["ASN:", $.lodash_get(body, "asn") || "-"]
              .filter((i) => i)
              .join(" ")
          : undefined,
      ]
    ...
    try {
      ...
      PROXY_INFO = [
        [
          ...
          .filter((i) => i)
          .join(" "),
        ["运营商:", $.lodash_get(body, "connection.isp") || "-"]
          .filter((i) => i)
          .join(" "),
        $.lodash_get(arg, "ORG") == 1
          ? ["组织:", $.lodash_get(body, "connection.org") || "-"]
              .filter((i) => i)
              .join(" ")
          : undefined,

        $.lodash_get(arg, "ASN") == 1
          ? ["ASN:", $.lodash_get(body, "connection.asn") || "-"]
              .filter((i) => i)
              .join(" ")
          : undefined,
      ]
      ...
      if (!ip && $.lodash_get(arg, "PRIVACY") == 1) {
        ...
        Object.keys(securityObj).forEach((key) => {
          security.push(
            `${key.toUpperCase()}: ${securityMap[securityObj[key]]}`
          );
        });
    ...
    try {
      ...
      PROXY_INFO = [
        [
          ...
          body.country.replace(/\s*中国\s*/, ""),
          body.regionName
            ? body.regionName.split(/\s+or\s+/)[0]
            : body.regionName,
          body.city,
        ]
          .filter((i) => i)
        ...
        $.lodash_get(arg, "ASN") == 1
          ? ["ASN:", $.lodash_get(body, "as") || "-"].filter((i) => i).join(" ")
          : undefined,
      ]
        .filter((i) => i)
        .join("\n");
...
async function getProxyInfoIPv6(ip) {
  let PROXY_IPv6;
  const msg = `使用 ${
    $.lodash_get(arg, "LANDING_IPv6") || "ipsb"
  } 查询 IPv6 分流信息`;
...
async function ipim(ip) {
  ...
  const province =
    body.match(/(^|\s+)Province\s*(:|：)\s*(.*)/m)?.[3] ||
    body.match(/(^|\s+)Region\s*(:|：)\s*(.*)/m)?.[3];
  ...
  INFO = [
    [
      "位置:",
      isCN ? getflag("CN") : getflag(country),
      country,
      province,
      city,
      district,
...
function maskAddr(addr) {
  ...
  if ($.lodash_get(arg, "MASK") == 1) {
    ...
    if (parts.length >= 3) {
      ...
      result =
        addr.substring(0, third) +
        "*".repeat(third) +
        addr.substring(2 * third);
    }
    return result;
  } else {
    return addr;
...
async function notify(title, subt, desc, opts) {
  if (
    $.lodash_get(arg, "TYPE") === "EVENT" ||
    $.lodash_get(arg, "notify") == 1
  ) {
